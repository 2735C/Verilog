...
module command_to_btn (
    ...
    // command data latch (1 cycle delay for rx_done)
    always @(posedge clk or posedge rst) begin
        if (rst)
            r_rx_data <= 8'd0;
        else if (rx_done_command)
            r_rx_data <= rx_data_command;
    end
        ...
        case (c_state)
            ...
            PROCESS: begin
                case (r_rx_data)
                    8'h47, 8'h67: n_state = RUN_STATE;         // 'G', 'g'
                    8'h43, 8'h63: n_state = CLEAR_STATE;       // 'C', 'c'
                    8'h53, 8'h73: n_state = STOP_STATE;        // 'S', 's'
                    8'h4E, 8'h6E: n_state = WATCH_MODE_STATE;  // 'N', 'n'
                    8'h4D, 8'h6D: n_state = MODE_STATE;        // 'M', 'm'
                    8'h1B:        n_state = RESET_STATE;       // 'ESC'
                    8'h52, 8'h72: n_state = R_STATE;           // 'R', 'r'
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            o_clear      <= (n_state == CLEAR_STATE);
            o_run        <= (n_state == RUN_STATE);
            o_stop       <= (n_state == STOP_STATE);
            o_watch_mode <= (n_state == WATCH_MODE_STATE);
            o_mode       <= (n_state == MODE_STATE);
            o_up         <= (n_state == U_STATE);
            o_down       <= (n_state == D_STATE);
            o_left       <= (n_state == L_STATE);
            o_right      <= (n_state == R_STATE);
            o_esc        <= (n_state == RESET_STATE);
        end
    end
